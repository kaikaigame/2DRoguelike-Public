...
public abstract class MovingObject : MonoBehaviour
{
    ...
    //Move takes parameters for x direction, 
    //y direction and a RaycastHit2D to check collision.
    protected bool Move(int xDir, int yDir, out RaycastHit2D hit)
    {
        //Store start position to move from, 
        //based on objects current transform position.
        Vector2 start = transform.position;

        //Calculate end position based on the 
        //direction parameters passed in when calling Move.
        Vector2 end = start + new Vector2(xDir, yDir);

        //Disable the boxCollider so that linecast 
        //doesn't hit this object's own collider.
        boxCollider.enabled = false;

        //Cast a line from start point to end point 
    ...
    protected IEnumerator SmoothMovement(Vector3 end)
    {
        ...
        //square magnitude of the difference between 
        //current position and end parameter. 
        //Square magnitude is used instead of magnitude 
        //because it's computationally cheaper.
        float sqrRemainingDistance = (transform.position - end).sqrMagnitude;

        //While that distance is greater than a very 
        //small amount (Epsilon, almost zero):
        while (sqrRemainingDistance > float.Epsilon)
        {
            //Find a new position proportionally closer 
            //to the end, based on the moveTime
            Vector3 newPostion = Vector3.MoveTowards(rb2D.position, 
                end, inverseMoveTime * Time.deltaTime);

            //Call MovePosition on attached Rigidbody2D and 
            ...
            //Return and loop until sqrRemainingDistance is 
            //close enough to zero to end the function
            yield return null;
        }
    }
    ...
    //of component we expect our unit to interact with 
        ...
        where T : Component
    {
        ...
        //Get a component reference to the component of 
        //type T attached to the object that was hit
